...
class glMesh(Mesh):
    def __init__(self, vertices, normals, vertex_colors, faces):
        super().__init__(vertices, normals, vertex_colors, faces)
        self._v_writes = set()
        self._n_writes = set()
        self._c_writes = set()
        self._v_refs = tuple([
    ...
    def _push_writes(self, write_list, buffer):
        # For float32 3-tuple buffers only
        ...
        for g in write_groups:
            ...
            glBufferSubData(GL_ARRAY_BUFFER, first * 12, g_data.nbytes, g_data)
